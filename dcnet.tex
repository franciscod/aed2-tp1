\begin{tad}{\tadNombre{DCNet}}
	...
	\tadOperacion{buscarPaquete}{dcnet/dcn, conj(pc)/pcs, nat/id}{pc}{pcs $\subseteq$ compus(topo(dcn)) $\land$ id $\in$ ids(paquetesEnLaRed(dcn))}
	...
	\tadOperacion{darPrioridad}{dcnet/dcn,nat/id}{nat}{id $\in$ ids(paquetesEnLaRed(dcn))}
	...
	\tadAxioma{existePaqEnBuffers?(dcn, pcs, id)}{
		...
		ELSE {
			\IF id $\in$ ids(buffer(dcn, dameUno(pcs))) THEN true
