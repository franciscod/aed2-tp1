\begin{tad}{\tadNombre{DCNet}}
	...
	\tadIgualdadObservacional{d}{d'}{dcnet}{$ (topo(d) \igobs topo(d')) \land
		((\forall p: pc)(p \in pcs(topo(d)) \land p \in pcs(topo(d')) \impluego (dcNetBuffer(d,p) \igobs dcNetBuffer(d',p) \land paquetesMandados(d,p) \igobs paquetesMandados(d',p) )
		\land ((\forall p: paquetes)
		((\exists  c: pc )(c \in pcs(topo(d') \land  c \in pcs(topo(d') ) \yluego(p \in dcNetBuffer(d,c) \land p \in dcNetBuffer(d',c) ) )
	...
	\tadObservadores
	\tadOperacion{recorridoPaquete}{dcnet/dcn,paquete/p}{secu((ip,interface)))}{$(\exists  c: pc )(c \in pcs(topo(dcn)) \yluego (p \in dcNetBuffer(dcn,c))$}
	...
	\tadOperacion{damePaquete}{buffer}{paquete}{}
	...
	\tadAxioma{paquetesMandados(crearRed(t),c)}{0}
	\tadAxioma{paquetesMandados(seg(dcn),c)}{paquetesMandados(dcn)}
	...
	\tadAxioma{maxPaquetesMandados(dcn)}{auxMaxPaquetes(dcn,pcs(topo(dcn)))}
	\tadAxioma{auxMaxPaquetes(dcn,cs)}{\IF $\emptyset?(sinUno(cs))$ THEN 
											$dameUno(cs)$ 
										ELSE { \IF $paquetesMandados(dcn,dameUno(cs)<$
													$paquetesMandados(dcn,auxMaxPaquetes$
													$(dcn,sinUno(cs))) $ THEN
	...
	\tadAxioma{cronoPaquetes(crearRed(t),bs)}{<bs>}
	\tadAxioma{cronoPaquetes(seg(dcn),bs)}{ bs \puntito cronoPaquetes(dcn,$\emptyset$) }
	\tadAxioma{cronoPaquetes(paquetePendiente(dcn,o,d,p),bs)}{ \IF $def?(c,bs)$ THEN 
																	$cronoPaquetes(dcn,definir(c,n \cup obtener(o,bs),bs))$
																ELSE 
																	$cronoPaquetes(dcn,definir(c,n))$ 
																FI}
	...
	\tadAxioma{envio(t,bs,b)}{\IF $\emptyset?(damePaquete(b))$ THEN
								$bs$
								ELSE{ \IF $pasarA(t,\Pi_1(b),dest(\Pi_2(b))) = destino(damePaquete(b)))$ THEN
											$envio(t,quitarPaquete(bs,\Pi_1(b)),(\Pi_1(b),\Pi_2(b) -{damePaquete(b)}))$
									    ELSE
											$envio(t,quitarPaquete(pasarPaquete(bs,\Pi_1(b),damePaquete(b)),\Pi_1(b))$
											$,(\Pi_1(b),\Pi_2(b) -{damePaquete(b)} ))$
										FI}
								FI}
