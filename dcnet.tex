\begin{tad}{\tadNombre{DCNet}}
	...
	{$(ipOrigen(p) \in compus(topo(dcn)) \land  ipDestino(p) \in compus(topo(dcn)) \yluego  conectadas?(topo(dcn) , ipOrigen(p) , ipDestino(p))$}
	...
	\tadAxioma{recorridoPaquete(dcn, p)}{
		cortarRecHasta(darCaminoMasCorto(topo(dcn), \\origen(p), destino(p)), buscarPaquete(compus(topo(dcn)), p))
	}
	...
	\tadAxioma{buscarPrioridad($idPaq$, $cs$)}{\IF $idPaq$ = id(dameUno($cs$)) THEN prioridad(dameUno($cs$)) ELSE buscarPrioridad($idPaq$, sinUno($cs$)) FI}
