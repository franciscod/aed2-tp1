\begin{tad}{\tadNombre{DCNet}}
	...
	\tadOperacion{darPrioridad}{nat id}{nat}{$id \in buffers(dcn)$ }
	...
	\tadOperacion{buscarPaquete}{dcnet/dcn, conj(nat)/compus, nat/id}{nat}{compus = compus(topo(dcn)) $\land$ ($\exists$ ip : nat)(ip $\in$ compus $\land$ id $\in$ buffer(dcn, ip))}
	...
	\tadAxioma{darPrioridad(seg(dcn,id))}{darPrioridad(dcn,id)}
	\tadAxioma{darPrioridad(CrearPaquete(dcn,c,c',p),id)}{\IF $id = \Pi_1(p)$ THEN $\Pi_4(p)$ ELSE $darPrioridad(dcn,id)$ FI}
	...
	\tadAxioma{buscarPaquete(dcn, compus, id)}{
		\IF id $\in$ buffer(dcn, dameUno(compus)) THEN dameUno(compus)
		ELSE buscarPaquete(sinUno(compus), id)
		FI
	}
	
	\tadAxioma{paqueteEnTransito?(dcn, id)}{
		existePaqueteEnBuffers?(dcn, compus(topo(dcn)), id)
	}
	\tadAxioma{existePaqueteEnBuffers?(dcn, pcs, id)}{
		\IF $\emptyset$?(pcs) THEN false
		ELSE {
			\IF id $\in$ buffer(dcn, dameUno(pcs)) THEN true
			ELSE existePaqueteEnBuffers?(dcn, sinUno(pcs), id)
			FI
		}
		FI
	}
	
	\tadAxioma{darPaqueteEnviado(cp)}{dameUno(PaquetesConPriopedadK(cp,maxPrioridad(cp)))}
	\tadAxioma{maxPrioridad(cp)}{max(darPrioridad(dameUno(cp),maxPrioridad(sinUno(cp)))}
	\tadAxioma{PaquetesConPriopedadK(cp,k)}{	\IF $\emptyset?cp)$ THEN 
													$\emptyset$
												ELSE{\IF $ darPrioridad(dameUno(cp)) = k$ THEN 	
														$ Ag(dameUno(cp),PaquetesConPriopedadK(sinUno(cp),k))$
													ELSE
														$PaquetesConPriopedadK(sinUno(cp),k)$
													FI}		
											FI}
