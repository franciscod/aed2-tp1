\begin{tadx}{\tadNombre{Topología}}{
	...
	\tadOperacion{Cable}{topologia, pc/ipA, nat/ifA, pc/ipB, nat/ifB}{topologia}{
	...
	\tadOperacion{cablesEn}{topologia/t, pc/ip}{conj(tupla(pc, nat))}{$ip \in compus(t)$}
	\tadOperacion{\#interfaces}{topologia/t, pc/ip}{nat}{$ip \in compus(t)$}

	\tadOtrasOperaciones

	\tadOperacion{vecinas}{topologia/t, pc/ip}{conj(pc)}{$ip \in compus(t)$}
	\tadOperacion{interfacesOcupadasDe}{topologia/t, pc/ip}{conj(nat)}{$ip \in compus(t)$}
	\tadOperacion{conectadas?}{topologia/t, pc/ipA, pc/ipB}{bool}{$ipA \in compus(t) $ $\land$ $ipB \in compus(t)$}
	\tadOperacion{buscaSegmento}{pc/ipA, pc/ipB, conj(segmento)}{segmento}{$ipB \in vecinas(ipA)$}
	...
	\tadOperacion{darRutas}{topologia, pc/ipA, pc/ipB, conj(nat), {secu(segmento)}}{conj(secu(segmento))}{$ipA \in compus(t) \land ipB \in compus(t)$}
	...
	\tadOperacion{$\pi_1$Conj}{conj({tupla(pc, nat)})}{conj(pc)}{}
	\tadOperacion{$\pi_2$Conj}{conj({tupla(pc, nat)})}{conj(nat)}{}
	...
	\tadAxioma{buscaSegmento($ipA$, $ipB$, $ss$)}{
		\IF ($ipA$ = $\pi_1$(dameUno($ss$))) $\land$ ($ipB$ = $\pi_3$(dameUno($ss$))) THEN
			dameUno($ss$)
		ELSE
			buscaSegmento($ipA$, $ipB$, sinUno($ss$))
		FI
	}

	\tadAxioma{darSegmento($t$, $ipA$, $ipB$)}{
		buscaSegmento($ipA$, $ipB$, segmentos($t$))
	}
	...
	\tadAlinearAxiomas{segmentos(Cable($t$, $ipA$, $ifA$, $ipB$, $ifB$))}

	\tadAxioma{segmentos(NuevaTopo)}{$\emptyset$}
	\tadAxioma{segmentos(Compu($t$, $ip$, $cantIfaces$))}{segmentos($t$)}
	\tadAxioma{segmentos(Cable($t$, $ipA$, $ifA$, $ipB$, $ifB$))}{
		Ag($\langle ipA, ifA, ipB, ifB \rangle$, Ag($\langle ipB, ifB, ipA, ifA \rangle$, segmentos($t$)))
	}
